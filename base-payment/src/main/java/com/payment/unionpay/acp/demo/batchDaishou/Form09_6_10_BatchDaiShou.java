 ...
 *       3）批量代收文件示例DK00000000777290058110097201510130001I.txt，注意：使用的时候需修改文件内容的批次号，日期（与txnTime前八位相同）总笔数，总金额等于下边参数中batchNo，txnTime，totalQty，totalAmt设定的一致。
 */
public class Form09_6_10_BatchDaiShou extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		contentData.put("txnSubType", "06");           			//填写06：代收
		...
		/**与批量文件内容相关的参数**/
		contentData.put("batchNo", batchNo);            			//批量交易时填写，当天唯一,0001-9999，商户号+批次号+上交易时间确定一笔交易
		contentData.put("txnTime", txnTime);  					    //前8位需与文件中的委托日期保持一致
		contentData.put("totalQty", totalQty);             			//批量交易时填写，填写批量文件中总的交易比数		
		contentData.put("totalAmt", totalAmt);           			//批量交易时填写，填写批量文件中总的交易金额
		
		//使用DEFLATE压缩算法压缩后，Base64编码的方式传输经压缩编码的文件内容，文件中的商户号必须与merId一致
		//示例文件位置在src/assets下
		contentData.put("fileContent", AcpService.enCodeFileContent(filePath, DemoBase.encoding));
