...
public class Form05_6_4_Refund extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		/***要调通交易以下字段必须修改***/
		if(!"".equals(origQryId)) data.put("origQryId", origQryId);      				 //****原消费交易返回的的queryId，可以从消费交易后台通知接口中或者交易状态查询接口中获取
		if(!"".equals(origOrderId)) data.put("origOrderId", origOrderId);      			 //****原消费交易返回的的orderId
		if(!"".equals(origTxnTime)) data.put("origTxnTime", origTxnTime);      			 //****原消费交易返回的的queryId，可以从消费交易后台通知接口中或者交易状态查询接口中获取
