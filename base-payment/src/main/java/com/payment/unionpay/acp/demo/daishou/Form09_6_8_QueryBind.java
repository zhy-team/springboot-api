 ...
 *                          4） 切换生产环境要点请FAQ搜索“切换”
 * 交易说明:本交易的用途一般是用卡号和绑定标识号确定存在关系或者根据卡号获取绑定标识号   
 */
public class Form09_6_8_QueryBind extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		contentData.put("txnType", "75");                              //交易类型 75-查询绑定关系
		...
		contentData.put("accNo", accNo);				   			   //若只送accNo,不送bindId那么会将改商户号下该卡号所有的绑定关系查询出来,两者都送会根据  商户号+卡号+绑定标识码  查询
		contentData.put("bindId", bindId);					           //若只送bindId,查该商户号下改bindId
